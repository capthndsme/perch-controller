...
// Default the tooltip's position animation OFF. Recharts animates the
...
function ChartTooltipContent({
  ...
  // Honor the same `itemSorter` API the default Recharts tooltip
  // exposes. The default `DefaultTooltipContent` sorts the payload
  // before mapping it, but custom content like ours receives the raw
  // payload from Recharts — so we have to apply the sort here.
  // Accepts the same shape Recharts does: a string key, or a function
  // returning a sort key (smaller comes first).
  const sortedPayload = React.useMemo(() => {
    if (!payload?.length || !itemSorter) return payload ?? []
    const compute =
      typeof itemSorter === "function"
        ? itemSorter
        : (item: (typeof payload)[number]) =>
            (item as unknown as Record<string, unknown>)[
              itemSorter as string
            ] as number | string
